...
# 6H：minimum match point = 4
# 1-5H：minimum match point = 10
MIN_MATCH_COUNT = 4
count_num = 0
save_path = "/tf/算法组/detect_images/BT0981优化图片/6H/6H_Affine_Results"
save_path_crop = "/tf/算法组/detect_images/BT0981优化图片/6H/6H_Crop_Results"
...
for filename in glob.glob("/tf/算法组/detect_images/BT0981优化图片/6H/*.png"):
    ...
    template_image = cv2.imread("/tf/算法组/detect_images/BT0981优化图片/6H/6H_Template/Template_6H.png", 0)
    #img2 = cv2.imread("F:\\Semantic-Segmentation-Suite-master\\DeepLearningTools\\AOI\\4H\\" + filename, 0)
    count_num = count_num + 1
    # Using SIFT detect feature points
    sift = cv2.xfeatures2d.SIFT_create()
    # sift=cv2.SIFT()

    # Obtain key points and key description
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        affined_image_bb = cv2.rectangle(affined_image, (245, 131), (982, 289), (255,255,255), 2)
        #1H: (303, 193), (1146, 420)
        #2H: (276, 172), (1413, 489)
        #3H: (292, 147),  (1020, 323)
        #4H: (172, 164)， (987, 379)
...
for result_file in glob.glob("/tf/算法组/detect_images/BT0981优化图片/6H/*.pngresult.png"): # move the result images to the "results" folder
    shutil.move(result_file,"/tf/算法组/detect_images/BT0981优化图片/6H/6H_Affine_Results")
    
for result_file in glob.glob("/tf/算法组/detect_images/BT0981优化图片/6H/*.pngcropresult.png"): # move the result images to the "results" folder
    shutil.move(result_file,"/tf/算法组/detect_images/BT0981优化图片/6H/6H_Crop_Results")
